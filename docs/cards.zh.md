...
- **用命令：** `.import <卡文件> [coc7] [pc|companion|world]`。
...
- **`keys` 与 `aliases` 的分工（npc 条目）。** `keys` 是把条目拉进主持人上下文的触发词——天然宽泛（地点、动作，甚至"守卫"这类泛称都可以）。独立的 `aliases` 字段只放**这个角色本人应答的名字**（简称、昵称、其他语言拼写），被玩家侧提及高亮与名字解析消费；绝不要把泛称放进去——否则玩家随口说"守卫"会错误高亮到某个 NPC 的卡片。
...
- **角色数值要过校验。** 导入的卡会变成当前规则系统（CoC 7 版）里的一张真卡，由规则包钳位、校验。
