...
**启用演出资料包它才会醒**：只有当房间启用了 `ui/presentation.yaml` 资料包，导演才上工。没有启用资料包的桌子从不唤醒它，也就不会为它付钱。出图还要额外满足三个条件，缺一不可：
...
- `.reset chars`——顺便重掷角色，模组留着。预设角色（含房间自建的）也留着，供重新认领。
- `.reset all`——全部抹掉。预设角色在这里才清空，模组自带的要恢复就重新导入模组。

想知道某个人物是模组自带的还是房间自建的、怎么删多余的预设角色，见 `modules.zh.md` 的“预设角色的来源与清理”。
