...
async def test_native_card_system_field_pins_room_system(tmp_path, user_rulepack_dir):
    """A native bundle that declares `system: wod` (generated by the forge's "directly use a
    built-in system" option, no rulepack shipped) pins that system on world import — the room
    then runs standard wod, and no rulepack generation is involved."""
    services = _services(tmp_path)
    card_path = _install_native_card(tmp_path / "data", system="wod")
    ctx = _keeper_ctx(tmp_path, "native-room")
    await CharcardTools(services).import_world_card(ctx, file_path=card_path)
    assert await services.store.state_get("native-room", "room_system") == "wod"
    assert (await services.room_rulepack(ctx)).system == "wod"
